...
while (x == 1):
    ...
    for n in range(0,5):
        ...
        top = lista_pts.pop(indice)
        lixo = lista_locais.pop(indice)
        mais_provavel_pts.append(top)
        mais_provavel.append(lista_locais[indice])

    for (local, pts) in zip(mais_provavel,mais_provavel_pts):
